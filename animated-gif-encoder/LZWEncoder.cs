...
namespace animated_gif_encoder
{
    ...
    internal class LZWEncoder
    {
      ...
      0x03FF, 0x07FF, 0x0FFF, 0x1FFF, 0x3FFF, 0x7FFF, 0xFFFF };
        ...
        void char_out(byte c, MemoryStream outs)
        {
            accum[a_count++] = c;
            if (a_count >= 254)
                flush_char(outs);
        ...
        void cl_block(MemoryStream outs)
        {
            cl_hash(hsize);
        ...
        void compress(int init_bits, MemoryStream outs)
        {
            int fcode;
            int i /* = 0 */;
            int c;
        ...
        // ----------------------------------------------------------------------------
        public void encode(MemoryStream os)
        {
        ...
        void flush_char(MemoryStream outs)
        {
            if (a_count > 0)
            {
        ...
        void output(int code, MemoryStream outs)
        {
